<template>
	...
	<view class="hx-navbar">
		...
		<view :class="{'hx-navbar--fixed': fixed,'hx-navbar--shadow':shadow,'hx-navbar--border':border,'global-Header-Color':true}"
		 :style="{'background': backgroundColorRgba}" class="hx-navbar__content">
			...
			<view :style="{color:colorInfo,height: height,'line-height':height}" class="hd hx-navbar__header hx-navbar__content_view">
				<view class="hx-navbar__header-btns hx-navbar__content_view" @tap="onClickLeft" v-if="leftSlot" :style="{'color': colorInfo}">
					<block v-if="leftText.length || leftIcon.length || back">
						<view v-if="leftIcon.length || back" :class="back ? 'left_back' : ''" class="hx-navbar__content_view">
							<uni-icons :type="back ? 'arrowleft' : leftIcon" :color="colorInfo" size="28" />
						</view>
						<view v-if="leftText.length" :class="{'hx-navbar-btn-icon-left':!leftIcon.length}" class="hx-navbar-btn-text hx-navbar__content_view">{{ leftText }}</view>
					</block>
					<block>
				...
				<view class="hx-navbar__header-container hx-navbar__content_view">
					<view v-if="title.length" class="hx-navbar__header-container-inner hx-navbar__content_view">{{ title }}</view>
					<!-- 标题插槽 -->
				...
				<view :class="title.length?'hx-navbar__header-btns-right':''" class="hx-navbar__header-btns hx-navbar__content_view"
				 @tap="onClickRight" v-if="rightSlot">
					<!-- 优先显示图标 -->
		...
		<view v-if="placeholder" class="hx-navbar__placeholder">
	...
	export default {
		...
		props: {
			...
			statusBarFontColor: {
				type: [Array, String],
				default: function() {
					return new Array("#FFFFFF", "#FFFFFF");
				}
			},
			//是否包含状态栏
			statusBar: {
		...
		methods: {
				...
				if (this.back) {
					if (getCurrentPages().length > 1) {
						...
						if (this.defaultBackUrl) {
							...
							console.log("222", this.defaultBackUrl);
						} else {
							if (this.backTabbarUrl) {
								wx.reLaunch({
									url: this.curToHome
								});
								console.log("333", this.backTabbarUrl);
							}
			...
			setBgColor() {
				...
				if (this.backgroundColor) {
					...
					that.backgroundColorRgba = 'rgba(' + rgbStr + ',' + 1 + ')';
				}
			},
