	...
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto"
	 :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" z-index="999">
		<view class="u-picker-header" @touchmove.stop.prevent="stop" catchtouchmove="stop">
			...
			<view class="u-btn-picker u-btn-picker--primary" hover-class="u-opacity" :hover-stay-time="150" @touchmove.stop=""
			 @tap.stop="getResult('confirm')">确定</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async loadProvinces() { // 加载省份
				let res = await getProvinces(1)
				
				this.districtsObj.provinces = res.data.data
				this.loadCities(res.data.data[0].id)
			},
			async loadCities(id) {
				let res = await getProvinces(id)
				this.districtsObj.cities = res.data.data
				this.loadAreas(res.data.data[0].id)
			},
			async loadAreas(id) {
				let res = await getProvinces(id)
				this.districtsObj.areas = res.data.data
			},
			...
			getResult(event = null) {
				let are;
				if(this.districtsObj.areas[this.area]){
					are=this.districtsObj.areas[this.area].district
				}else{
					are=[]
				}
				let result = {
					province: this.districtsObj.provinces[this.province].district,
					city: this.districtsObj.cities[this.city].district,
					area: are
				}
