<template>
    <image :src="imageSrc||''" @error="handleLoadError" :class="{'setIcon':setIcon}"></image>
</template>
    ...
    export default{
        ...
        methods:{
            async handleCache(src){
                const cachedRes = await uni.getStorage({key: src })
